...
function buildScene(THREE, host, {
  ...
  const pushRibbon = (ptsA, ptsB, opts) => {
    ...
    for (let i = 0; i < n; i++) {
      const pA = ptsA[i],
        pB = ptsB[i];
      const tA = tangentAt(ptsA, i),
        tB = tangentAt(ptsB, i);
      const v = i / (n - 1);
      const w = opts.halfW * (0.70 + 0.30 * sstep(0.0, 0.12, v)) * mix(1, opts.tipTaper, sstep(0.70, 1.0, v));
  ...
  for (let i = 0; i < strands; i++) {
    ...
    const tipY = isFly ? FOCUS_Y + 0.15 - rnd() * 0.9 : FOCUS_Y - (0.50 + c.lenVar * 0.13 + (rnd() - 0.5) * 0.06);
    ...
    for (let k = 0; k < KA; k++) {
      ...
      scalp.push(new THREE.Vector3(HEAD_C[0] + r * Math.sin(phi) * Math.sin(a), HEAD_C[1] + r * Math.cos(phi) * 1.15, HEAD_C[2] + r * Math.sin(phi) * Math.cos(a) * 1.05));
    }
    ...
    for (let k = 1; k <= KB; k++) {
      ...
      rawA.push(new THREE.Vector3(E.x * (1 - 0.32 * v) + wobX + jx * tight + dirOut * flickAmt * Math.pow(v, 3.2), E.y + (tipY - E.y) * v, mix(E.z, zTarget, Math.pow(v, 1.05)) + wobZ + jz * tight + flickAmt * 0.4 * Math.pow(v, 3.0)));
    }
    ...
    for (let k = 1; k <= KB; k++) {
      ...
      rawB.push(new THREE.Vector3(E.x * (1 - 0.26 * v) + wobX + jx * tight + dirOut * flickAmt * 1.6 * Math.pow(v, 2.8), E.y + (tipYB - E.y) * v, mix(E.z, zTarget + 0.03, Math.pow(v, 1.02)) + wobZ + jz * tight + flickAmt * 0.9 * Math.pow(v, 2.6)));
    }
    pushRibbon(resample(rawA, 21), resample(rawB, 21), {
      halfW,
      tipTaper: isFly ? 0.15 : 0.5,
      color: tone,
      rand: [c.cr, rnd()],
      stagger: rnd()
    });
  ...
  const apply = () => {
    ...
    camera.position.set(0.16 * t1, mix(0.56, FOCUS_Y + 0.10, t1) - 0.04 * creep, mix(wideZ, closeZ, t1) - 0.12 * creep);
